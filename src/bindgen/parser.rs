        ...
        for item in items {
            ...
            match item {
                syn::Item::ForeignMod(ref item) => {
                    self.load_syn_foreign_mod(binding_crate_name, crate_name, top_level_items_outside_of_binding_crate, mod_cfg, item);
                }
                syn::Item::Fn(ref item) => {
                    self.load_syn_fn(binding_crate_name, crate_name, top_level_items_outside_of_binding_crate, mod_cfg, item);
                }
                syn::Item::Const(ref item) => {
                    self.load_syn_const(binding_crate_name, crate_name, top_level_items_outside_of_binding_crate, mod_cfg, item);
                }
                syn::Item::Static(ref item) => {
                    self.load_syn_static(binding_crate_name, crate_name, top_level_items_outside_of_binding_crate, mod_cfg, item);
                }
